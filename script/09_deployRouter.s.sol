...
contract RouterScript is Script {
    ...
    function run() public {
        ...
        router_selector[1] = Router(address(0)).setKyberRouter.selector;
        router_selector[2] = Router(address(0)).setPendleRouter.selector;
        vm.startBroadcast();
        ...
        if (!forTest) {
            ...
            envVar = string.concat("PENDLE_ROUTER_ADDRESS_", deploymentNetwork);
            if (bytes(vm.envString(envVar)).length == 0) {
                revert(string.concat(envVar, " is not set in .env file"));
            }
            pendleRouter = vm.envAddress(envVar);
        }
    ...
    function deployForTest(
        ...
        public
        returns (address _testRouterRes, address _testRouterUtilRes, address _testCurveLiqArbitrage)
    {
        forTest = true;
        registry = _registry;
