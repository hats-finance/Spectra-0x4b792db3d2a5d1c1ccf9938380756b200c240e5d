...
contract CustomDecimals is Test {
    ...
    function testCurveLiqArbitrageFuzz(
        ...
        // 0.8e18 initial price is hardcoded in deployAll script
        // The function previewUnitaryAddLiquidity does not seem to be strictly unimodal in cases of small deposits
        if (depositAmount < 5 * 10 ** ibtDecimals) {
            assertApproxEqRel(data.baseProportion1, 0.8e18, 5e15, "base proportion is wrong 1");
        } else {
            assertApproxEqRel(data.baseProportion1, 0.8e18, 1e15, "base proportion is wrong 2");
        }
